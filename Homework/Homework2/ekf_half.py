...
print(f"\n=== Part (F): using only first half of arc (EKF) ===")
...
# -----------------------------
# 4) A priori covariance P0 (ORIGINAL)
# -----------------------------
sigma_r0_km = 1.0
...
# IMPORTANT: interpolate truth at the SAME times as the measurements you are processing
Xtrue_meas = np.column_stack([np.interp(t_meas, truth_times, truth_states[:,k]) for k in range(6)])


# -----------------------------
# 7) Dynamics parameters (same as batch)
# -----------------------------
mu = 398600.4418
J2 = 1.08262668e-3
J3 = -2.532153e-6
...
# -----------------------------
# 8) Run EKF  (FUNCTION CALL)  (FIRST HALF ONLY)
# -----------------------------
...
# -----------------------------
# 9) Plot: state error vs time (±3σ if your EKF returns two_sigma_meas)
# -----------------------------
t = out["t_meas"]
...
fig.suptitle("EKF State Error vs Time (First Half Only)", y=0.98)
plt.tight_layout()
...
# -----------------------------
post = out["postfit_resids_meas"]  # (m,2): [rho, rho_dot]
...
fig.suptitle("EKF Postfit Residuals vs Time (First Half Only)", y=0.98)
plt.tight_layout()
...
print("\n=== RMS Summary (EKF, First Half Only) ===")
print(df_rms.to_string(index=False))


# -----------------------------
# 12) Zoom plot (t >= 4000 s)
# NOTE: for first-half-only you may not have any samples beyond 4000.
# -----------------------------
t0_zoom = 4000.0
mask = t >= t0_zoom
...
if np.any(mask):
    ...
    handles, leglabels = axs[0].get_legend_handles_labels()
    fig.legend(handles, leglabels, loc="upper right")
    fig.suptitle("EKF State Error vs Time (Zoom: t ≥ 4000 s) [First Half Only]", y=0.98)
    plt.tight_layout()
else:
