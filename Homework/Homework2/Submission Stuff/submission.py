...
BATCH_PLOTS_DIR = SCRIPT_DIR / "Plots" / "Batch for B"
...
# Load truth IC from the truth trajectory CSV first row
...
PLOTS_DIR = Path("Plots") / "LKF for part D"     # <- subfolder for LKF figures
...
SCRIPT_DIR = Path(__file__).resolve().parent
PLOTS_DIR = SCRIPT_DIR / "Plots" / "EKF CS E"  
PLOTS_DIR.mkdir(parents=True, exist_ok=True)
...
plt.tight_layout()
save_fig(fig, "coldstart_state_error_all.png")
# -----------------------------
# 10) Plot: postfit residuals with ±3σ measurement noise bounds
# -----------------------------
post = out["postfit_resids_meas"]  # (m,2)
...
plt.tight_layout()
save_fig(fig, "coldstart_postfit_residuals.png")
...
plt.tight_layout()
save_fig(fig, f"coldstart_state_error_zoom_t_ge_{int(t0_zoom)}.png")
