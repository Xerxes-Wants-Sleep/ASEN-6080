...
from src.Functions.filters import LinearizedKalmanFilter   # <-- change if your module name differs
...
print(f"\n=== Part (F): using only first half of arc (LKF) ===")
...
# -----------------------------
# 3) Measurement noise R  (ORIGINAL)
# -----------------------------
...
PLOTS_DIR = Path("Plots") / "LKF for part F"
PLOTS_DIR.mkdir(parents=True, exist_ok=True)


# -----------------------------
# 4) A priori covariance P0  (ORIGINAL P0)
# -----------------------------
sigma_r0_km = 1.0
...
# -----------------------------
# 5) Process noise Q (DISCRETE)
# -----------------------------
Q = np.zeros((6, 6), dtype=float)


# -----------------------------
# 6) Truth + initial guess X0_star = X0_true + dx  (ORIGINAL dx)
# -----------------------------
...
# IMPORTANT: build truth at the SAME times as the measurements you are processing (first-half only)
...
# -----------------------------
# 8) Run LKF  (FUNCTION CALL)  (FIRST HALF ONLY)
# -----------------------------
...
# -----------------------------
# 9) Plot: state error vs time (±3σ if your LKF returns two_sigma_meas)
# -----------------------------
t = out["t_meas"]
...
fig.suptitle("LKF State Error vs Time (First Half Only)", y=0.98)
plt.tight_layout()

save_fig(fig, "lkf_state_error_first_half.png")
...
fig.suptitle("LKF Postfit Residuals vs Time (First Half Only)", y=0.98)
plt.tight_layout()

save_fig(fig, "lkf_postfit_residuals_first_half.png")
...
print("\n=== RMS Summary (LKF, First Half Only) ===")
print(df_rms.to_string(index=False))


# -----------------------------
# 12) Plot: state error vs time (Zoom: t >= 4000 s)
# NOTE: with first-half-only, you may not even have t >= 4000.
# -----------------------------
t0_zoom = 4000.0
mask = t >= t0_zoom
...
if np.any(mask):
    ...
    handles, leglabels = axs[0].get_legend_handles_labels()
    fig.legend(handles, leglabels, loc="upper right")
    fig.suptitle("LKF State Error vs Time (Zoom: t ≥ 4000 s) [First Half Only]", y=0.98)
    plt.tight_layout()

    save_fig(fig, "lkf_state_error_zoom_t_ge_4000_first_half.png")
else:
    print("Zoom plot skipped: no measurement times with t >= 4000 s in first-half-only dataset.")
