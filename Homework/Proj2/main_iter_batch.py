...
def main():
    ...
    # Prefer linearized postfit if present; otherwise fall back to nonlinear postfit.
    if ("postfit_resids_linear_hist" in info) and len(info["postfit_resids_linear_hist"]) > 0:
        postfit_linear = np.asarray(info["postfit_resids_linear_hist"][-1], dtype=float)
    else:
        postfit_linear = np.asarray(info["postfit_residuals"], dtype=float)

    out_batch = {
        "t_meas": np.asarray(info["t_meas"], dtype=float),
        "station_meas": list(info["station_meas"]),
        "xhat_meas": np.asarray(info["state_hist"], dtype=float),
        "P_meas": np.asarray(info["P_hist"], dtype=float),
        "prefit_resids_final": np.asarray(info["prefit_residuals"], dtype=float),
        "postfit_resids_linear_final": postfit_linear,
        "postfit_resids_meas": np.asarray(info["postfit_residuals"], dtype=float),
        "state_error_meas": None,
        "rms_final": {},
        "rms_by_iter": None,
        "R": np.asarray(R_batch, dtype=float),
    }

    print(f"Iterated Batch Run Complete. Number Of Updates: {len(out_batch['t_meas'])}")
    print(f"Estimated Cr At Epoch: {float(x0_hat[6]):.9f}")

    # Convert km->m for plotting helpers that label covariance in SI units.
    ...
    outdir = Path(__file__).resolve().parent / "Plots" / "Main Iterated Batch"
    ...
    make_final_state_estimate_plot(out_batch, outdir)

    print_final_state_summary("Iterated Batch", out_batch["xhat_meas"][-1])
