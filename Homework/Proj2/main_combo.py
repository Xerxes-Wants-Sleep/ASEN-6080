import argparse
import numpy as np
import pandas as pd
import sys
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[2]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.append(str(PROJECT_ROOT))

from src.Functions.batch import batch_estimate_x0
from src.Functions.itekf_2 import IEKF2
from src.Functions.propagation import PropSettings
from src.Functions.stations import Stations
from src.Functions.srp_dyn_model import mu_sun_srp_state_deriv
from src.Functions.jacobians import srp_thirdbody_variational_eq
from src.Functions.Ephem import ephem
from src.helpers.plotting.post_processing import run_filter_post_processing
from src.helpers.plotting.plot_postfit_residuals_linear import make_postfit_residuals_linear_plot
from src.helpers.plotting.plot_state_estimate_3sigma import make_all_state_3sigma_envelope_plot


def build_stations():
    theta0_deg = 0
    w_earth_rad_per_s = 7.29211585275553e-5
    radius_earth_km = 6378.1363

    return [
        Stations(
            "DSS 34",
            lat_deg=-35.398333,
            lon_deg=148.981944,
            theta0_deg=theta0_deg,
            radius_earth=radius_earth_km + 0.691750,
            w_earth_rad_per_s=w_earth_rad_per_s,
        ),
        Stations(
            "DSS 65",
            lat_deg=40.427222,
            lon_deg=355.749444,
            theta0_deg=theta0_deg,
            radius_earth=radius_earth_km + 0.834539,
            w_earth_rad_per_s=w_earth_rad_per_s,
        ),
        Stations(
            "DSS 13",
            lat_deg=35.247164,
            lon_deg=243.205000,
            theta0_deg=theta0_deg,
            radius_earth=radius_earth_km + 1.07114904,
            w_earth_rad_per_s=w_earth_rad_per_s,
        ),
    ]


def build_problem_constants():
    Jd0 = 2456296.25
    mu_sun = 132712440017.987
    AU_km = 149597870.7
    solar_flux_W_m2 = 1357.0
    SRP_area_mass_ratio = 0.01

    pConst = type("pConst", (), {})()
    pConst.mu_earth = 398600.4415
    pConst.mu_sun = mu_sun

    scConst = type("scConst", (), {})()
    scConst.area = SRP_area_mass_ratio
    scConst.mass = 1.0
    scConst.solar_flux_1au = solar_flux_W_m2
    scConst.c = 299792458.0
    scConst.AU_m = AU_km * 1000.0

    def earth_state_func(tau):
        return np.zeros(3, dtype=float), np.zeros(3, dtype=float)

    def sun_state_func(tau):
        jd = Jd0 + float(tau) / 86400.0
        rE_km, vE_km_s, _ = ephem(jd, 3, frame="EME2000")
        rE_km = np.asarray(rE_km, dtype=float).reshape(3,)
        vE_km_s = np.asarray(vE_km_s, dtype=float).reshape(3,)
        return -rE_km, -vE_km_s

    return pConst, scConst, earth_state_func, sun_state_func


def build_dyn_and_jac(pConst, scConst, earth_state_func, sun_state_func):
    dyn = lambda tau, x: mu_sun_srp_state_deriv(
        t=tau,
        X=x,
        pConst=pConst,
        scConst=scConst,
        earth_state_func=earth_state_func,
        sun_state_func=sun_state_func,
    )

    def jac(tau, x):
        x = np.asarray(x, dtype=float).reshape(-1)
        r_earth, _ = earth_state_func(tau)
        r_sun, _ = sun_state_func(tau)
        return srp_thirdbody_variational_eq(
            r_sc=x[0:3],
            r_earth=r_earth,
            r_sun=r_sun,
            Cr=float(x[6]),
            area=scConst.area,
            mass=scConst.mass,
            mu_earth=pConst.mu_earth,
            mu_i=pConst.mu_sun,
            solar_flux_1au=scConst.solar_flux_1au,
            c=scConst.c,
            AU_m=scConst.AU_m,
        )

    return dyn, jac


def load_project2_obs(obs_path: Path) -> list[dict]:
    df = pd.read_csv(
        obs_path,
        skipinitialspace=True,
        index_col=False,
        usecols=range(7),
    )
    df.columns = [c.strip() for c in df.columns]

    col_time = "Time since Epoch"
    station_cols = {
        "DSS 34": ("DSS34 Range (km)", "DSS34 Range-Rate (km/sec)"),
        "DSS 65": ("DSS65 Range (km)", "DSS65 Range-Rate (km/sec)"),
        "DSS 13": ("DSS13 Range (km)", "DSS13 Range-Rate (km/sec)"),
    }

    pieces = []
    for st_name, (rho_col, rhod_col) in station_cols.items():
        part = df[[col_time, rho_col, rhod_col]].rename(
            columns={
                col_time: "t",
                rho_col: "rho_km",
                rhod_col: "rho_dot_km_s",
            }
        )
        part["station"] = st_name
        pieces.append(part)

    meas = pd.concat(pieces, ignore_index=True)
    meas["t"] = pd.to_numeric(meas["t"], errors="coerce")
    meas["rho_km"] = pd.to_numeric(meas["rho_km"], errors="coerce")
    meas["rho_dot_km_s"] = pd.to_numeric(meas["rho_dot_km_s"], errors="coerce")
    meas = meas.dropna(subset=["rho_km", "rho_dot_km_s"])
    meas = meas.sort_values("t")[["station", "t", "rho_km", "rho_dot_km_s"]]
    return meas.to_dict(orient="records")


def batch_info_to_filter_like_out(info: dict, R_km: np.ndarray) -> dict:
    xhat = np.asarray(info["state_hist"], dtype=float)
    P = np.asarray(info["P_hist"], dtype=float)
    two_sigma = 2.0 * np.sqrt(np.maximum(np.diagonal(P, axis1=1, axis2=2), 0.0))

    prefit = np.asarray(info.get("prefit_residuals", np.full((xhat.shape[0], 2), np.nan)), dtype=float)
    postfit_nl = np.asarray(info.get("postfit_residuals", np.full((xhat.shape[0], 2), np.nan)), dtype=float)

    postfit_lin = None
    if "postfit_resids_linear_hist" in info and len(info["postfit_resids_linear_hist"]) > 0:
        postfit_lin = np.asarray(info["postfit_resids_linear_hist"][-1], dtype=float)
    if postfit_lin is None:
        postfit_lin = postfit_nl.copy()

    return {
        "t_meas": np.asarray(info["t_meas"], dtype=float),
        "station_meas": list(info["station_meas"]),
        "xhat_meas": xhat,
        "Xhat_meas": xhat,
        "P_meas": P,
        "two_sigma_meas": two_sigma,
        "prefit_resids_final": prefit,
        "postfit_resids_linear_final": postfit_lin,
        "postfit_resids_meas": postfit_nl,
        "state_error_meas": None,
        "R": np.asarray(R_km, dtype=float),
        "rms_final": None,
        "rms_by_iter": None,
    }


def combine_filter_out(a: dict, b: dict | None) -> dict:
    if b is None:
        return dict(a)

    out = dict(b)
    arr_keys = [
        "t_meas",
        "xhat_meas",
        "Xhat_meas",
        "P_meas",
        "two_sigma_meas",
        "prefit_resids_final",
        "postfit_resids_linear_final",
        "postfit_resids_meas",
    ]
    for k in arr_keys:
        if (k in a) and (k in b) and (a[k] is not None) and (b[k] is not None):
            out[k] = np.concatenate([np.asarray(a[k]), np.asarray(b[k])], axis=0)

    if ("station_meas" in a) and ("station_meas" in b):
        out["station_meas"] = list(a["station_meas"]) + list(b["station_meas"])

    out["state_error_meas"] = None
    out["R"] = np.asarray(a.get("R", b.get("R")), dtype=float)
    out["rms_final"] = b.get("rms_final", None)
    out["rms_by_iter"] = b.get("rms_by_iter", None)
    return out


def to_plotting_result_6state_from_filter_km(out: dict, *, R_km: np.ndarray):
    xhat = np.asarray(out["xhat_meas"], dtype=float)
    P = np.asarray(out["P_meas"], dtype=float)

    xhat6_m = xhat[:, 0:6] * 1000.0
    P6_m = P[:, 0:6, 0:6] * (1000.0**2)
    two_sigma6_m = 2.0 * np.sqrt(np.maximum(np.diagonal(P6_m, axis1=1, axis2=2), 0.0))

    d = {
        "t_meas": np.asarray(out["t_meas"], dtype=float),
        "station_meas": list(out["station_meas"]),
        "xhat_meas": xhat6_m,
        "Xhat_meas": xhat6_m,
        "P_meas": P6_m,
        "two_sigma_meas": two_sigma6_m,
        "state_error_meas": None,
        "prefit_resids_final": np.asarray(out["prefit_resids_final"], dtype=float) * 1000.0,
        "postfit_resids_linear_final": np.asarray(out["postfit_resids_linear_final"], dtype=float) * 1000.0,
        "postfit_resids_meas": np.asarray(out["postfit_resids_meas"], dtype=float) * 1000.0,
        "rms_final": out.get("rms_final", {}),
        "rms_by_iter": out.get("rms_by_iter", None),
        "R": np.asarray(R_km, dtype=float) * (1000.0**2),
    }
    return run_filter_post_processing(out=d)


def make_plot_set(out: dict, outdir: Path, *, R_km: np.ndarray):
    outdir.mkdir(parents=True, exist_ok=True)
    postfit_plot_out = to_plotting_result_6state_from_filter_km(out, R_km=R_km)
    make_postfit_residuals_linear_plot(postfit_plot_out, outdir)
    make_all_state_3sigma_envelope_plot(out, outdir)


def make_smoothed_section_out(out: dict) -> dict:
    """
    Build a plotting-compatible smoothed output dictionary.

    If true smoothed keys exist (Xhat_smooth/P_smooth), use them.
    Otherwise, fall back to the section output itself (batch fixed-interval style).
    """
    out_s = dict(out)

    if ("Xhat_smooth" in out) and ("P_smooth" in out):
        x_s = np.asarray(out["Xhat_smooth"], dtype=float)
        p_s = np.asarray(out["P_smooth"], dtype=float)
        out_s["xhat_meas"] = x_s
        out_s["Xhat_meas"] = x_s
        out_s["P_meas"] = p_s
        out_s["two_sigma_meas"] = 2.0 * np.sqrt(
            np.maximum(np.diagonal(p_s, axis1=1, axis2=2), 0.0)
        )
        if out.get("postfit_resids_linear_smooth", None) is not None:
            out_s["postfit_resids_linear_final"] = np.asarray(out["postfit_resids_linear_smooth"], dtype=float)
        if out.get("postfit_resids_smooth_nl", None) is not None:
            out_s["postfit_resids_meas"] = np.asarray(out["postfit_resids_smooth_nl"], dtype=float)
        elif out.get("postfit_resids_linear_smooth", None) is not None:
            out_s["postfit_resids_meas"] = np.asarray(out["postfit_resids_linear_smooth"], dtype=float)
        if out.get("state_error_smooth_meas", None) is not None:
            out_s["state_error_meas"] = np.asarray(out["state_error_smooth_meas"], dtype=float)
        return out_s

    # Fallback: for fixed-interval batch-style section, treat final section output as smoothed.
    out_s["xhat_meas"] = np.asarray(out["xhat_meas"], dtype=float)
    out_s["Xhat_meas"] = np.asarray(out["Xhat_meas"], dtype=float)
    out_s["P_meas"] = np.asarray(out["P_meas"], dtype=float)
    out_s["two_sigma_meas"] = np.asarray(out["two_sigma_meas"], dtype=float)
    return out_s


def main():
    parser = argparse.ArgumentParser(description="Run 0-10 day iterated batch, then IEKF2 for the remaining arc.")
    parser.add_argument("--batch-days", type=float, default=15.0, help="Days for the initial iterated batch section.")
    parser.add_argument("--batch-max-iter", type=int, default=10, help="Max iterations for iterated batch.")
    parser.add_argument("--batch-tol", type=float, default=1.0e-10, help="Convergence tolerance for iterated batch.")
    parser.add_argument("--iekf-max-iter", type=int, default=10, help="Max measurement-iteration count in IEKF2.")
    parser.add_argument("--iekf-iter-tol", type=float, default=1.0e-10, help="Measurement-iteration tolerance in IEKF2.")
    args = parser.parse_args()

    base = Path(__file__).resolve().parent
    obs_path = base / "Given_data" / "Project2b_Obs.txt"
    all_meas = load_project2_obs(obs_path)
    all_meas = sorted(all_meas, key=lambda m: float(m["t"]))
    stations = build_stations()

    pConst, scConst, earth_state_func, sun_state_func = build_problem_constants()
    dyn_fun, dyn_jac = build_dyn_and_jac(pConst, scConst, earth_state_func, sun_state_func)

    x0 = np.array(
        [
            -274096770.76544,
            -92859266.4499061,
            -40199493.6677441,
            32.6704564599943,
            -8.93838913761049,
            -3.87881914050316,
            1,
        ],
        dtype=float,
    )
    P0 = np.diag([100.0**2, 100.0**2, 100.0**2, 0.1**2, 0.1**2, 0.1**2, 0.1**2])

    sigma_rho_km = 5.0e-3
    sigma_rhod_km_s = 0.5e-6
    R = np.diag([sigma_rho_km**2, sigma_rhod_km_s**2])

    sigma_acc_km_s2 = 1.0e-9  #SNC
    Q_iekf = np.diag([sigma_acc_km_s2**2, sigma_acc_km_s2**2, sigma_acc_km_s2**2])

    split_t = float(args.batch_days) * 86400.0
    meas_batch = [m for m in all_meas if float(m["t"]) <= (split_t + 1.0e-12)]
    meas_iekf = [m for m in all_meas if float(m["t"]) > (split_t + 1.0e-12)]
    if len(meas_batch) == 0:
        raise ValueError("No measurements selected for initial iterated batch section.")

    x0_hat, P0_hat, info_batch = batch_estimate_x0(
        all_meas=meas_batch,
        stations=stations,
        x0_bar=x0,
        P0=P0,
        R=R,
        mu=pConst.mu_earth,
        J2=0.0,
        J3=0.0,
        Re=6378.1363,
        max_iter=int(args.batch_max_iter),
        tol=float(args.batch_tol),
        reltol=1.0e-10,
        abstol=1.0e-10,
        method="RK45",
        dyn_fun=dyn_fun,
        dyn_jac=dyn_jac,
        prop_settings=PropSettings(rtol=1.0e-10, atol=1.0e-10, method="RK45"),
    )
    out_batch = batch_info_to_filter_like_out(info_batch, R_km=R)
    print(
        f"Initial Iterated Batch Complete: {len(out_batch['t_meas'])} Measurements "
        f"(0.0 To {float(out_batch['t_meas'][-1]) / 86400.0:.3f} Days)"
    )

    out_iekf = None
    out_iekf_smooth = None
    if len(meas_iekf) > 0:
        x_handoff = np.asarray(info_batch["state_hist"][-1], dtype=float).copy()
        P_handoff = np.asarray(info_batch["P_hist"][-1], dtype=float).copy()
        t_handoff = float(info_batch["t_meas"][-1])

        newekf = IEKF2(
            x0=x_handoff,
            P0=P_handoff,
            R=R,
            Q=Q_iekf,
            dyn_fun=dyn_fun,
            dyn_jac=dyn_jac,
            prop_settings=PropSettings(rtol=1.0e-10, atol=1.0e-10, method="RK45"),
            first_pass_gap_s=6 * 3600.0,
        )
        out_iekf = newekf.run(
            all_meas=meas_iekf,
            stations=stations,
            Xtrue_meas=None,
            t_prev_init=t_handoff,
            iterated=True,
            max_iter=int(args.iekf_max_iter),
            iter_tol=float(args.iekf_iter_tol),
            bound_level=5.0,
            no_snc_on_first_after_gap=True,
            gap_threshold_s=6 * 3600.0,
            show_progress=True,
            progress_every=50,
        )
        newekf.smooth(out_iekf, stations=stations)
        out_iekf_smooth = make_smoothed_section_out(out_iekf)
        print(
            f"IEKF Section Complete: {len(out_iekf['t_meas'])} Measurements "
            f"({float(out_iekf['t_meas'][0]) / 86400.0:.3f} To {float(out_iekf['t_meas'][-1]) / 86400.0:.3f} Days)"
        )

    out_total = combine_filter_out(out_batch, out_iekf)
    out_batch_smooth = make_smoothed_section_out(out_batch)

    plot_root = base / "Plots" / "Main Combo"
    ilkf_section_root = plot_root / "ILKF Section"
    ilkf_smooth_section_root = plot_root / "ILKF Section Smoothed"
    ekf_section_root = plot_root / "EKF Section"
    ekf_smooth_section_root = plot_root / "EKF Section Smoothed"
    total_root = plot_root / "Total"
    ilkf_section_root.mkdir(parents=True, exist_ok=True)
    ilkf_smooth_section_root.mkdir(parents=True, exist_ok=True)
    ekf_section_root.mkdir(parents=True, exist_ok=True)
    ekf_smooth_section_root.mkdir(parents=True, exist_ok=True)
    total_root.mkdir(parents=True, exist_ok=True)

    make_plot_set(out_batch, ilkf_section_root, R_km=R)
    make_plot_set(out_batch_smooth, ilkf_smooth_section_root, R_km=R)
    if out_iekf is not None:
        make_plot_set(out_iekf, ekf_section_root, R_km=R)
    if out_iekf_smooth is not None:
        make_plot_set(out_iekf_smooth, ekf_smooth_section_root, R_km=R)
    make_plot_set(out_total, total_root, R_km=R)

    xf = np.asarray(out_total["xhat_meas"][-1], dtype=float).reshape(-1)
    print("\nFinal State Estimate (Main Combo):")
    print(f"  Position [km]    : [{xf[0]:.6f}, {xf[1]:.6f}, {xf[2]:.6f}]")
    print(f"  Velocity [km/s]  : [{xf[3]:.9f}, {xf[4]:.9f}, {xf[5]:.9f}]")
    print(f"  Cr [-]           : {xf[6]:.9f}")
    print(f"Saved Smoothed ILKF-Section Plots To: {ilkf_smooth_section_root}")
    if out_iekf_smooth is not None:
        print(f"Saved Smoothed EKF-Section Plots To: {ekf_smooth_section_root}")
    print(f"\nSaved Combo Plots To: {plot_root}")


if __name__ == "__main__":
    main()
