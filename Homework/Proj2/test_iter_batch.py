...
def batch_info_to_filter_like_out(info: dict, R_km: np.ndarray) -> dict:
    ...
    prefit = None
    if "prefit_resids_hist" in info and len(info["prefit_resids_hist"]) > 0:
        prefit = np.asarray(info["prefit_resids_hist"][-1], dtype=float)
    elif "prefit_residuals" in info:
        prefit = np.asarray(info["prefit_residuals"], dtype=float)
    ...
    postfit_nl = None
    if "postfit_residuals" in info:
        postfit_nl = np.asarray(info["postfit_residuals"], dtype=float)
...
def run_iter_batch_once(
    ...
    *,
    ...
    out = batch_info_to_filter_like_out(info, R_km=R)
...
def main():
    ...
    out_root = base / "Validation Plots" / "Test Iter Batch"
    out_root.mkdir(parents=True, exist_ok=True)

    # Full iterated-batch run over all 2a observations
    x0_hat_full, P0_hat_full, info_full, out_full = run_iter_batch_once(
        all_meas=all_meas,
        stations=stations,
        x0_bar=x0,
        P0=P0,
    ...
    print(f"Full Iter-Batch Run Complete. Number Of Updates: {len(out_full['t_meas'])}")
    make_postfit_plot(out_full, out_root, R_km=R_batch)
    make_state_estimate_plot(out_full, out_root)

    xf_full = np.asarray(out_full["xhat_meas"][-1], dtype=float)
    print("\nFinal State Estimate (Full Iter-Batch):")
    ...
    print(f"\nSaved Iter-Batch Test Plots To: {out_root}")
