...
def main():
    ...
    print(f"Iterated EKF2 Run Complete. Number Of Updates: {len(out_iter['t_meas'])}")
    make_postfit_plot(out_iter, iter_root, R_km=R_ekf)
    make_state_estimate_plot(out_iter, iter_root)

    xf_reg = np.asarray(out_regular["xhat_meas"][-1], dtype=float)
    xf_itr = np.asarray(out_iter["xhat_meas"][-1], dtype=float)
    print("\nFinal State Estimate (Regular EKF2):")
    ...
    print("\nFinal State Estimate (Iterated EKF2):")
    ...
    print(f"\nSaved Test New EKF Plots To: {out_root}")
