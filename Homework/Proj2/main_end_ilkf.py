...
def main():
    parser = argparse.ArgumentParser(description="Run ILKF on the last N days of Project2b data.")
    ...
    if sigma_acc <= 0.0:
        Q_lkf = np.zeros((7, 7), dtype=float)
    else:
    ...
    # Smoothing API differs across LKF wrapper/delegate paths.
    if hasattr(lkf, "smooth"):
        lkf.smooth(run_out=out, stations=stations)
    elif hasattr(lkf, "_delegate") and hasattr(lkf._delegate, "smooth"):
        lkf._delegate.smooth(run_out=out, stations=stations)
    out_smooth = make_smoothed_plot_out(out)

    day0 = float(out["t_meas"][0]) / 86400.0
    day1 = float(out["t_meas"][-1]) / 86400.0
    outdir = base / "Plots" / "End ILKF" / f"Days_{int(np.floor(day0)):03d}_{int(np.ceil(day1)):03d}"
    ...
    print("\nFinal State Estimate (End-Arc ILKF):")
    ...
    if out_smooth is not None:
        ...
        print("\nFinal State Estimate (Smoothed End-Arc ILKF):")
        print(f"  Position [km]    : [{xfs[0]:.6f}, {xfs[1]:.6f}, {xfs[2]:.6f}]")
        print(f"  Velocity [km/s]  : [{xfs[3]:.9f}, {xfs[4]:.9f}, {xfs[5]:.9f}]")
        print(f"  Cr [-]           : {xfs[6]:.9f}")
    print(f"\nSaved End-Arc ILKF Plots To: {outdir}")
    if out_smooth is not None:
        print(f"Saved Smoothed End-Arc ILKF Plots To: {outdir_smooth}")
