...
def build_smoothed_plot_out(out_smooth: dict) -> dict:
...
def main():
    ...
    out_root = Path(__file__).resolve().parent / "Plots" / "Main Broken IEKF"
    ...
    for arc_idx, day_end in enumerate(arc_days, start=1):
        ...
        if not _is_finite_state(x0_current):
            raise RuntimeError(f"Arc {arc_idx} received non-finite x0_current: {x0_current}")
        ...
        out_smooth_plot = build_smoothed_plot_out(out_smooth)

        x0_next = np.asarray(out_fwd["Xhat_meas"][-1], dtype=float).copy()
        P0_next = np.asarray(out_fwd["P_meas"][-1], dtype=float).copy()
        if not _is_finite_state(x0_next):
            x0_fallback = np.asarray(out_smooth_plot["xhat_meas"][0], dtype=float).copy()
            if _is_finite_state(x0_fallback):
                print(
                ...
                P0_next = np.asarray(P0_ekf, dtype=float).copy()

        arc_dir = out_root / f"Arc_{arc_idx:02d}_{day_end:07.3f}_Days"
        make_standard_plot_set(out_fwd, arc_dir / "Forward Iterated EKF")
        make_standard_plot_set(out_smooth_plot, arc_dir / "Smoothed Back")

        iter_counts = np.asarray(out_fwd.get("iter_counts", []), dtype=float)
        summary_rows.append(
            {
                "arc_index": arc_idx,
                "arc_end_days": day_end,
                "num_meas": len(meas_arc),
                "iter_mean": float(np.nanmean(iter_counts)) if iter_counts.size else np.nan,
                "iter_max": int(np.nanmax(iter_counts)) if iter_counts.size else np.nan,
                "iter_min": int(np.nanmin(iter_counts)) if iter_counts.size else np.nan,
        ...
        print(
            ...
            f"Iter Mean={summary_rows[-1]['iter_mean']:.3f}, "
            f"Iter Max={summary_rows[-1]['iter_max']}"
        )
        ...
        final_smooth = out_smooth_plot

    whole_dir = out_root / "Whole Run"
    if final_forward is not None and final_smooth is not None:
        make_standard_plot_set(final_forward, whole_dir / "Forward Iterated EKF")
        make_standard_plot_set(final_smooth, whole_dir / "Smoothed Back")
        print_final_state_summary("Broken IEKF Whole Run (Smoothed)", final_smooth["xhat_meas"][-1])
