...
def extract_case_from_json_block(block: dict, *, smoothed: bool):
    ...
    if x_key not in block or p_key not in block or t_key not in block:
        raise ValueError(
            f"JSON missing required keys ({x_key}, {p_key}, {t_key}). "
            "Re-run main_iekf_stop_start.py to regenerate the final JSON."
        )

    x = np.asarray(block[x_key], dtype=float).reshape(-1)
    P = np.asarray(block[p_key], dtype=float)
    t0 = float(block[t_key])
...
def _resolve_history_npz_path(base: Path, payload: dict) -> Path | None:
    ...
    p = base / "Plots" / "IEKF Stop Start" / "iekf_stop_start_history_for_end_batch.npz"
...
def main():
    ...
    json_path = base / "Plots" / "IEKF Stop Start" / "final_states_iekf_stop_start.json"
    outdir = base / "Plots" / "Part3 BPlane IEKF Stop Start"
    outdir.mkdir(parents=True, exist_ok=True)

    with open(json_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    stop_start_block = payload.get("iekf_stop_start_6state", payload.get("newekf_6state", payload.get("maneuver_iekf", {})))
    if len(stop_start_block) == 0:
        raise ValueError(
            "Missing stop-start final-state block in final_states_iekf_stop_start.json. "
            "Expected one of: 'iekf_stop_start_6state', 'newekf_6state', or 'maneuver_iekf'."
        )

    p_const, sc_const, earth_state_func, sun_state_func = build_problem_constants()

    x6_f, P6_f, t0_f = extract_case_from_json_block(stop_start_block, smoothed=False)
    cases = [("Forward IEKF Stop-Start", x6_f, P6_f, t0_f)]
    ...
    if history_path is not None:
        ...
        history_specs = [("Forward IEKF Stop-Start", "xhat_meas", "P_meas")]
