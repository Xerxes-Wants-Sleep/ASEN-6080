...
x0_ukf = np.array([-274096790.0, -92859240.0, -40199490.0, 32.67,  -8.94, -3.88, 1.2], dtype=float)
P0_ukf = np.diag(
    [
        100.0**2,     # x [km]
        100.0**2,     # y [km]
...
# With n=7, provide full-state discrete Q (start with zero for baseline tuning).
...
print(f"UKF 2a Run Complete. Number Of Updates: {len(out_ukf_2a['t_meas'])}")
...
final_plot_dir = Path(__file__).resolve().parent / "Validation Plots"
final_plot_dir.mkdir(parents=True, exist_ok=True)

result_2a = to_plotting_result_6state_from_ukf_km(out_ukf_2a, R_km=R_ukf)
