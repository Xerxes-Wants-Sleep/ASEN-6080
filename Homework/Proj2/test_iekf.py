...
def main():
    ...
    all_meas_2a = load_project2_obs(obs_2a_path)
    ...
    print(f"IEKF 2a Run Complete. Number Of Updates: {len(out_iekf_2a['t_meas'])}")

    arc_days_target = [50.0, 100.0, 150.0, 200.0]
    t_days = np.asarray(out_iekf_2a["t_meas"], dtype=float) / 86400.0
    xhat = np.asarray(out_iekf_2a["xhat_meas"], dtype=float)
    P = np.asarray(out_iekf_2a["P_meas"], dtype=float)

    bdot_t_ref_km = 9796.737
    bdot_r_ref_km = 14970.824
    rows = []
    ...
    for d_target in arc_days_target:
        ...
        x_arc = xhat[idx, :]
        P_arc = P[idx, :, :]
        t_arc = float(out_iekf_2a["t_meas"][idx])
        ...
        print(
            ...
            f"errT={float(bdot_t_arc - bdot_t_ref_km):10.3f} km, "
            f"errR={float(bdot_r_arc - bdot_r_ref_km):10.3f} km"
        )
        ...
        rows.append(
            {
                ...
                "day_selected": d_sel,
                "BdotT_km": float(bdot_t_arc),
                "BdotR_km": float(bdot_r_arc),
                "BdotT_error_km": float(bdot_t_arc - bdot_t_ref_km),
                "BdotR_error_km": float(bdot_r_arc - bdot_r_ref_km),
                "sigma_T_km": float(sig_t_arc),
                "sigma_R_km": float(sig_r_arc),
            }
        )

    outdir = Path(__file__).resolve().parent / "Validation Plots"
    outdir.mkdir(parents=True, exist_ok=True)
    out_csv = outdir / "bplane_arc_summary_iekf.csv"
    pd.DataFrame(rows).to_csv(out_csv, index=False)
    print(f"Saved IEKF B-Plane Arc Summary: {out_csv}")
