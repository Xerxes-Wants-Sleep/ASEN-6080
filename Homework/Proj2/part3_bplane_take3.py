...
def extract_case_from_json(block: dict, *, smoothed: bool):
    ...
    if x_key not in block or p_key not in block or t_key not in block:
        raise ValueError(
            ...
            "Re-run main_newekf.py to regenerate final_states_newekf.json."
        )
...
def _resolve_history_npz_path(base: Path, payload: dict) -> Path | None:
    ...
    p = base / "Plots" / "New EKF" / "newekf_history_for_end_batch.npz"
...
def main():
    ...
    json_path = base / "Plots" / "New EKF" / "final_states_newekf.json"
    outdir = base / "Plots" / "Part3 BPlane Take 3"
    outdir.mkdir(parents=True, exist_ok=True)

    with open(json_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    out_block = payload.get("newekf_6state", payload.get("maneuver_iekf", {}))
    if len(out_block) == 0:
        raise ValueError("Missing 'newekf_6state' (or compatible 'maneuver_iekf') block in final_states_newekf.json.")

    p_const, sc_const, earth_state_func, sun_state_func = build_problem_constants()

    x6_f, P6_f, t0_f = extract_case_from_json(out_block, smoothed=False)
    cases = [("Forward IEKF 6-State", x6_f, P6_f, t0_f)]
    ...
    if history_path is not None:
        ...
        history_specs = [("Forward IEKF 6-State", "xhat_meas", "P_meas")]
