...
state0 = Xt[0, 0:7].astype(float).copy()
...
R_ilkf = np.diag([sigma_rho_km**2, sigma_rhod_km_s**2])

# Initial ILKF reference state/covariance scaffold for 7-state [r, v, Cr].
x0_ilkf = np.array([-274096790.0, -92859240.0, -40199490.0, 32.67,  -8.94, -3.88, 1.2], dtype=float)
P0_ilkf = np.diag(
    [
        100.0**2,     # x [km]
        100.0**2,     # y [km]
...
# With n=7, provide full-state discrete Q (baseline tune = zero).
# Q_ilkf = np.zeros((7, 7), dtype=float) #SNC
sigma_acc_km_s2 = 1.0e-10
Q_ilkf = np.diag([sigma_acc_km_s2**2, sigma_acc_km_s2**2, sigma_acc_km_s2**2])


dyn_ilkf = lambda tau, x: mu_sun_srp_state_deriv(
    t=tau,
    X=x,
    pConst=pConst,
    scConst=scConst,
...
ilkf_2a = LinearizedKalmanFilter(
    X0_star=x0_ilkf,
    P0=P0_ilkf,
    R=R_ilkf,
    Q=Q_ilkf,
...
out_ilkf_2a = ilkf_2a.run_iterated(
...
print(f"ILKF 2a Run Complete. Number Of Updates: {len(out_ilkf_2a['t_meas'])}")
...
final_plot_dir = Path(__file__).resolve().parent / "Validation Plots" / "testing_bplane"
final_plot_dir.mkdir(parents=True, exist_ok=True)

result_2a = to_plotting_result_6state_from_filter_km(out_ilkf_2a, R_km=R_ilkf)
make_prefit_residuals_plot(result_2a, final_plot_dir)
make_postfit_residuals_linear_plot(result_2a, final_plot_dir)

make_all_state_3sigma_envelope_plot(out_ilkf_2a, final_plot_dir)
make_cr_3sigma_plot(out_ilkf_2a, final_plot_dir)

# Part (h): B-Plane 3-Sigma Ellipses At 50/100/150/200 Day Arcs
arc_days_target = [50.0, 100.0, 150.0, 200.0]
t_days_ilkf = np.asarray(out_ilkf_2a["t_meas"], dtype=float) / 86400.0
xhat_ilkf = np.asarray(out_ilkf_2a["xhat_meas"], dtype=float)
P_ilkf = np.asarray(out_ilkf_2a["P_meas"], dtype=float)

bplane_records = []
for d_target in arc_days_target:
    idx = int(np.argmin(np.abs(t_days_ilkf - float(d_target))))
    x_arc = xhat_ilkf[idx, :]
    P_arc = P_ilkf[idx, :, :]
    t_arc = float(out_ilkf_2a["t_meas"][idx])
    d_sel = float(t_days_ilkf[idx])
