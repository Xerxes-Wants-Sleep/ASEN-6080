...
def main():
    ...
    # Q_ekf = np.zeros((7, 7), dtype=float)
    # SNC test value
    ...
    print(f"EKF Smoother Run Complete. Number Of Updates: {len(out_smooth['t_meas'])}")

    # Final EKF forward-pass plots (from smoother's internal forward pass)
    out_ekf_for_plots = dict(out_smooth)
    result_ekf = run_filter_post_processing_18(
        out=out_ekf_for_plots,
        length_unit_in="km",
        length_unit_out="m",
    )

    outdir_base = Path(__file__).resolve().parent / "Plots" / "Main EKF with Smoother"
    outdir_final = outdir_base / "Final EKF Forward"
    outdir_final.mkdir(parents=True, exist_ok=True)

    make_prefit_residuals_plot(result_ekf, outdir_final)
    make_postfit_residuals_linear_plot(result_ekf, outdir_final)
    make_trace_cov_plot(result_ekf, outdir_final)
    make_trace_cov_pos_vel_plot(result_ekf, outdir_final, length_unit="m")
    make_final_state_estimate_plot(out_ekf_for_plots, outdir_final)
    ...
    print_final_state_summary("EKF", out_smooth["xhat_meas"][-1])
    print(f"Saved Final EKF Plots To: {outdir_final}")
