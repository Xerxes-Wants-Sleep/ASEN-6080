...
def load_project2_obs(obs_path: Path) -> list[dict]:
    # Same reader pattern as tests.py
    df = pd.read_csv(
        obs_path,
...
def to_plotting_result_6state_from_filter_km(out: dict, *, R_km: np.ndarray):
    """
    Convert 7-state filter output in km/km-s to shared 6-state plotting format in m/m-s.
    """
...
def main():
    ...
    # Same data/IC setup as tests.py final section
    ...
    out_root = base / "Validation Plots" / "Test EKF"
    regular_root = out_root / "Regular EKF"
    full_iekf_root = out_root / "Full IEKF"
    chunk_root = out_root / "Chunked IEKF"
    regular_root.mkdir(parents=True, exist_ok=True)
    full_iekf_root.mkdir(parents=True, exist_ok=True)
    chunk_root.mkdir(parents=True, exist_ok=True)

    # ------------------------------------------------------------------
    # 1) Straight EKF over full dataset
    # ------------------------------------------------------------------
    ekf_full = make_ekf_filter(
        x0=x0,
        P0=P0,
        R=R_ekf,
        Q=Q,
    ...
    print(f"Regular EKF Run Complete. Number Of Updates: {len(out_regular['t_meas'])}")

    make_postfit_plot(out_regular, regular_root, R_km=R_ekf)
    make_state_estimate_plot(out_regular, regular_root)

    # ------------------------------------------------------------------
    # 2) Full IEKF over full dataset (iterated=True)
    # ------------------------------------------------------------------
    iekf_full = make_ekf_filter(
        x0=x0,
        P0=P0,
        R=R_ekf,
        Q=Q,
    ...
    print(f"Full IEKF Run Complete. Number Of Updates: {len(out_full_iekf['t_meas'])}")

    make_postfit_plot(out_full_iekf, full_iekf_root, R_km=R_ekf)
    make_state_estimate_plot(out_full_iekf, full_iekf_root)

    # ------------------------------------------------------------------
    # 3) 50-day chunked IEKF (iterated), handoff chunk-to-chunk
    # ------------------------------------------------------------------
    ekf_chunk = make_ekf_filter(
        x0=x0,
        P0=P0,
        R=R_ekf,
        Q=Q,
    ...
    while day_start < (max_day - 1.0e-12):
        ...
        make_postfit_plot(out_chunk, cdir, R_km=R_ekf)
    ...
    print(f"\nChunked IEKF Run Complete. Number Of Updates: {len(out_chunk_total['t_meas'])}")

    make_postfit_plot(out_chunk_total, chunk_root, R_km=R_ekf)
    make_state_estimate_plot(out_chunk_total, chunk_root)

    xf_reg = np.asarray(out_regular["xhat_meas"][-1], dtype=float)
    xf_full_itr = np.asarray(out_full_iekf["xhat_meas"][-1], dtype=float)
    xf_itr = np.asarray(out_chunk_total["xhat_meas"][-1], dtype=float)
    print("\nFinal State Estimate (Regular EKF):")
    ...
    print("\nFinal State Estimate (Full IEKF):")
    print(f"  Position [km]    : [{xf_full_itr[0]:.6f}, {xf_full_itr[1]:.6f}, {xf_full_itr[2]:.6f}]")
    print(f"  Velocity [km/s]  : [{xf_full_itr[3]:.9f}, {xf_full_itr[4]:.9f}, {xf_full_itr[5]:.9f}]")
    print(f"  Cr [-]           : {xf_full_itr[6]:.9f}")
    print("\nFinal State Estimate (Chunked IEKF):")
    print(f"  Position [km]    : [{xf_itr[0]:.6f}, {xf_itr[1]:.6f}, {xf_itr[2]:.6f}]")
    print(f"  Velocity [km/s]  : [{xf_itr[3]:.9f}, {xf_itr[4]:.9f}, {xf_itr[5]:.9f}]")
    print(f"  Cr [-]           : {xf_itr[6]:.9f}")
    print(f"\nSaved EKF Test Plots To: {out_root}")
