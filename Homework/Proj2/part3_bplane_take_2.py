...
def extract_case_from_maneuver_json(maneuver_block: dict, *, smoothed: bool):
    ...
    if x_key not in maneuver_block or p_key not in maneuver_block or t_key not in maneuver_block:
        raise ValueError(
            ...
            "Re-run main_maneuver_check.py to regenerate the final JSON."
        )
...
def _resolve_history_npz_path(base: Path) -> Path | None:
    p = base / "Plots" / "Maneuver Check IEKF" / "maneuver_check_history.npz"
...
def main():
    ...
    json_path = base / "Plots" / "Maneuver Check IEKF" / "final_states_maneuver_check.json"
    outdir = base / "Plots" / "Part3 BPlane Take 2"
    outdir.mkdir(parents=True, exist_ok=True)
    ...
    if len(maneuver_block) == 0:
        raise ValueError("Missing 'maneuver_iekf' block in final_states_maneuver_check.json.")
    ...
    cases = [("Forward IEKF", x6_f, P6_f, t0_f)]
    ...
    if history_path is not None:
        ...
        history_specs = [("Forward IEKF", "xhat_meas", "P_meas")]
