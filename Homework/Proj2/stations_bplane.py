...
def extract_case_from_maneuver_json(maneuver_block: dict, *, smoothed: bool):
    suffix = "smoothed" if smoothed else "forward"
    x_key = f"final_state_iekf_{suffix}"
    p_key = f"final_cov_iekf_{suffix}"
    t_key = f"final_time_s_iekf_{suffix}"

    if x_key not in maneuver_block or p_key not in maneuver_block or t_key not in maneuver_block:
        raise ValueError(
            f"JSON missing required keys ({x_key}, {p_key}, {t_key}). "
            "Re-run main_station_estimation_iekf.py to regenerate the final JSON."
        )

    x = np.asarray(maneuver_block[x_key], dtype=float).reshape(-1)
    P = np.asarray(maneuver_block[p_key], dtype=float)
    t0 = float(maneuver_block[t_key])

    if x.shape[0] < 6:
        raise ValueError(f"{x_key} must have at least 6 elements; got {x.shape}.")
    if P.shape[0] < 6 or P.shape[1] < 6:
        raise ValueError(f"{p_key} must be at least 6x6; got {P.shape}.")

    x6 = x[:6].copy()
    P6 = P[:6, :6].copy()
    if not np.all(np.isfinite(x6)) or not np.all(np.isfinite(P6)) or not np.isfinite(t0):
        raise ValueError(f"Non-finite values in {suffix} case state/cov/time.")
...
def make_bplane_plot(rec: dict, outdir: Path):
    ...
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(outdir / f"part3_bplane_crossings_3sigma_{_safe_label_for_filename(label)}.png", dpi=300, bbox_inches="tight")
    plt.close(fig)


def _resolve_history_npz_path(base: Path) -> Path | None:
    p = base / "Plots" / "Station IEKF" / "station_iekf_history.npz"
...
def main():
    ...
    json_path = base / "Plots" / "Station IEKF" / "final_states_station_iekf.json"
    outdir = base / "Plots" / "Part3 BPlane Station IEKF"
    outdir.mkdir(parents=True, exist_ok=True)

    with open(json_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    maneuver_block = payload.get("maneuver_iekf", {})
    if len(maneuver_block) == 0:
        raise ValueError("Missing 'maneuver_iekf' block in final_states_station_iekf.json.")

    p_const, sc_const, earth_state_func, sun_state_func = build_problem_constants()

    x6_f, P6_f, t0_f = extract_case_from_maneuver_json(maneuver_block, smoothed=False)
    cases = [("Forward Station IEKF", x6_f, P6_f, t0_f)]
    ...
    for label, x6, p6, t0 in cases:
        ...
        print(
            ...
            f"rp={rec['rp_km']:10.3f} km, "
            f"alt={rec['rp_alt_km']:10.3f} km"
        )
    ...
    history_path = _resolve_history_npz_path(base)
    arc_rows = []
    if history_path is not None:
        ...
        history_specs = [("Forward Station IEKF", "xhat_meas", "P_meas")]
