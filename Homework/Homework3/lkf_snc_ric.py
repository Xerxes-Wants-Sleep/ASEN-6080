...
def make_optimal_plots(out: dict, outdir: Path, R: np.ndarray, sigma_opt_m_s2: float) -> None:
    ...
    fig.tight_layout()
    fig.savefig(outdir / "lkf_optimal_state_errors_eci_ricq.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    if np.any(mask):
        ...
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
        fig.savefig(outdir / "lkf_optimal_state_errors_eci_zoom_t_ge_5h_ricq.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
...
def main():
    ...
    plot_dir = Path("Plots") / "LKF_SNC_RIC"
    ...
    summary_df.to_csv(plot_dir / "lkf_snc_ric_sweep_summary.csv", index=False)
    ...
    print(f"\nOptimal sigma (LKF RIC-Q, by minimum 3D position RMS): {sigma_opt_m_s2:.6e} m/s^2")
