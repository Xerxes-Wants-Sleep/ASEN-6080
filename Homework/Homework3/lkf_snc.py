...
def make_sweep_plots(summary_df: pd.DataFrame, outdir: Path, opt_sigma_m_s2: float) -> None:
    ...
    fig.suptitle("LKF SNC Sweep: Linear Postfit RMS vs sigma")
    fig.tight_layout()
    ...
    axs[1].grid(True, which="both")
    axs[1].legend(loc="best")
    fig.suptitle("LKF SNC Sweep: 3D State RMS vs sigma")
    fig.tight_layout()
...
def make_optimal_plots(out: dict, outdir: Path, R: np.ndarray, sigma_opt_m_s2: float) -> None:
    ...
    fig.suptitle(f"LKF Optimal SNC State Errors (+/-3sigma), sigma={sigma_opt_m_s2:.3e} m/s^2")
    ...
    if np.any(mask):
        ...
        axs[-2].set_xlabel("Time [hours]")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"LKF Optimal SNC State Errors (+/-3sigma), t >= 5 h, sigma={sigma_opt_m_s2:.3e} m/s^2")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
    ...
    fig.suptitle(f"LKF Optimal SNC Linear Postfit Residuals, sigma={sigma_opt_m_s2:.3e} m/s^2")
    fig.tight_layout()
