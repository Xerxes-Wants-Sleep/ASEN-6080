...
def make_sweep_plots(summary_df: pd.DataFrame, outdir: Path, opt_sigma_m_s2: float) -> None:
    ...
    fig.suptitle("LKF SNC Sweep: Linear Postfit RMS vs Process Noise Acceleration")
    fig.tight_layout()
    fig.savefig(outdir / "lkf_snc_sweep_postfit_rms.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    axs[1].grid(True, which="both")
    axs[1].legend(loc="best")
    fig.suptitle("LKF SNC Sweep: 3D State RMS vs Process Noise Acceleration")
    fig.tight_layout()
    fig.savefig(outdir / "lkf_snc_sweep_state_rms.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def make_optimal_plots(out: dict, outdir: Path, R: np.ndarray, sigma_opt_m_s2: float) -> None:
    ...
    fig.suptitle(f"LKF Optimal SNC State Errors (+/-3sigma), Process Noise Acceleration={sigma_opt_m_s2:.3e}")
    handles, leglabels = axs[0].get_legend_handles_labels()
    fig.legend(handles, leglabels, loc="upper right")
    fig.tight_layout()
    fig.savefig(outdir / "lkf_optimal_state_errors_eci.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    if np.any(mask):
        ...
        axs[-2].set_xlabel("Time [hours]")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"LKF Optimal SNC State Errors (+/-3sigma), t >= 5 h, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
        fig.savefig(outdir / "lkf_optimal_state_errors_eci_zoom_t_ge_5h.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    fig.suptitle(f"LKF Optimal SNC Linear Postfit Residuals, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
    fig.tight_layout()
    fig.savefig(outdir / "lkf_optimal_postfit_linear.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    best_idx = summary_df["pos3_rms_km"].to_numpy(float).argmin()
    sigma_opt_m_s2 = float(summary_df.iloc[best_idx]["sigma_m_s2"])

    summary_df.to_csv(plot_dir / "lkf_snc_sweep_summary.csv", index=False)
    make_sweep_plots(summary_df, plot_dir, sigma_opt_m_s2)
    ...
    print(f"\nOptimal sigma (LKF, by minimum 3D position RMS): {sigma_opt_m_s2:.6e} m/s^2")
