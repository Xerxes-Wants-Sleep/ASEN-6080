...
def load_problem2_inputs(mu_km3_s2: float):
    ...
    return all_meas, Xtrue_meas, stations, R, P0, x0_hat, x0_true, truth_times, truth_states
...
def make_sweep_plots(summary_df: pd.DataFrame, outdir: Path, opt_sigma_m_s2: float, tau_s: float) -> None:
    ...
    fig.suptitle(f"EKF DMC Sweep (tau={tau_s:.1f}s): Linear Postfit RMS vs Process Noise Acceleration")
    fig.tight_layout()
    ...
    axs[1].grid(True, which="both")
    axs[1].legend(loc="best")
    fig.suptitle(f"EKF DMC Sweep (tau={tau_s:.1f}s): 3D State RMS vs Process Noise Acceleration")
    fig.tight_layout()
...
def make_optimal_plots(
    ...
    fig.suptitle(f"EKF DMC Optimal State Errors (+/-3sigma), Process Noise Acceleration={sigma_opt_m_s2:.3e}, tau={tau_s:.1f}s")
    ...
    if np.any(mask):
        ...
        axs[-2].set_xlabel("Time [hours]")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal State Errors (+/-3sigma), t >= 5 h, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
    ...
    fig.suptitle(f"EKF DMC Optimal Linear Postfit Residuals, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
    fig.tight_layout()
    ...
    if X9.ndim == 2 and X9.shape[1] >= 9:
        ...
        for i in range(3):
            ...
            axs[i].grid(True)
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal w Estimate vs J3 Accel Truth, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
        ...
        if np.any(mask_w):
            ...
            axs[-1].set_xlabel("Time [hours]")
            fig.suptitle(
                f"EKF DMC Optimal w Estimate vs J3 Accel Truth, t >= 4 h, Process Noise Acceleration={sigma_opt_m_s2:.3e}"
            )
            handles, leglabels = axs[0].get_legend_handles_labels()
            fig.legend(handles, leglabels, loc="upper right")
            fig.tight_layout()
        ...
        if w_ref_km_s2 is not None and np.asarray(w_ref_km_s2).shape == w_hat.shape:
            ...
            for i in range(3):
                ...
                axs[i].grid(True)
            axs[-1].set_xlabel("Time [hours]")
            fig.suptitle(f"EKF DMC Optimal Accel Error, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
            fig.tight_layout()
            fig.savefig(diag_dir / "ekf_dmc_optimal_w_error.png", dpi=300, bbox_inches="tight")
            plt.close(fig)
    ...
    if qww.ndim == 2 and qww.shape[1] == 3 and qww.shape[0] == t_hr.shape[0]:
        ...
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal Interval Qww vs Time, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        fig.tight_layout()
    ...
    if P9.ndim == 3 and P9.shape[1] >= 9 and P9.shape[0] == t_hr.shape[0]:
        ...
        for i in range(3):
            ...
            axs[i].grid(True, which="both")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal Pww vs Time, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        fig.tight_layout()
