...
from src.Functions.dmc import ekf_with_dmc
...
def load_problem2_inputs(mu_km3_s2: float):
    ...
    truth_aligned = truth_df[truth_df["t_s"].isin(t_meas)].sort_values("t_s")
    if len(truth_aligned) == len(t_meas):
        Xtrue_meas = truth_aligned[["x_km", "y_km", "z_km", "vx_km_s", "vy_km_s", "vz_km_s"]].to_numpy(float)
    else:
        Xtrue_meas = np.column_stack([np.interp(t_meas, truth_times, truth_states[:, k]) for k in range(6)])

    # Match HW2 problem setup.
    r0_true, v0_true, _ = Keplarian_to_Cartesian(
        mu_km3_s2,
        10000.0,
        0.001,
        40.0,
    ...
    x0_true = np.hstack((r0_true, v0_true))
    ...
    return all_meas, Xtrue_meas, stations, R, P0, x0_hat, x0_true
...
def make_sweep_plots(summary_df: pd.DataFrame, outdir: Path, opt_sigma_m_s2: float, tau_s: float) -> None:
    ...
    fig.suptitle(f"EKF DMC Sweep (tau={tau_s:.1f}s): Linear Postfit RMS vs sigma")
    fig.tight_layout()
    fig.savefig(outdir / "ekf_dmc_sweep_postfit_rms.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    axs[1].grid(True, which="both")
    axs[1].legend(loc="best")
    fig.suptitle(f"EKF DMC Sweep (tau={tau_s:.1f}s): 3D State RMS vs sigma")
    fig.tight_layout()
    fig.savefig(outdir / "ekf_dmc_sweep_state_rms.png", dpi=300, bbox_inches="tight")
    plt.close(fig)


def make_optimal_plots(out: dict, outdir: Path, R: np.ndarray, sigma_opt_m_s2: float, tau_s: float) -> None:
    ...
    fig.suptitle(f"EKF DMC Optimal State Errors (+/-3sigma), sigma={sigma_opt_m_s2:.3e} m/s^2, tau={tau_s:.1f}s")
    handles, leglabels = axs[0].get_legend_handles_labels()
    fig.legend(handles, leglabels, loc="upper right")
    fig.tight_layout()
    fig.savefig(outdir / "ekf_dmc_optimal_state_errors_eci.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    if np.any(mask):
        ...
        axs[-2].set_xlabel("Time [hours]")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal State Errors (+/-3sigma), t >= 5 h, sigma={sigma_opt_m_s2:.3e} m/s^2")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
        fig.savefig(outdir / "ekf_dmc_optimal_state_errors_eci_zoom_t_ge_5h.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    fig.suptitle(f"EKF DMC Optimal Linear Postfit Residuals, sigma={sigma_opt_m_s2:.3e} m/s^2")
    fig.tight_layout()
    fig.savefig(outdir / "ekf_dmc_optimal_postfit_linear.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    if X9.ndim == 2 and X9.shape[1] >= 9 and P9.ndim == 3 and P9.shape[1] >= 9:
        ...
        for i in range(3):
            ...
            axs[i].grid(True)
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal w Estimate (+/-3sigma), sigma={sigma_opt_m_s2:.3e} m/s^2")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
        fig.savefig(diag_dir / "ekf_dmc_optimal_w_estimate.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
        ...
        if np.any(mask_w):
            ...
            axs[-1].set_xlabel("Time [hours]")
            fig.suptitle(
                f"EKF DMC Optimal w Estimate (+/-3sigma), t >= 4 h, sigma={sigma_opt_m_s2:.3e} m/s^2"
            )
            handles, leglabels = axs[0].get_legend_handles_labels()
            fig.legend(handles, leglabels, loc="upper right")
            fig.tight_layout()
            fig.savefig(diag_dir / "ekf_dmc_optimal_w_estimate_t_ge_4h.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    if qww.ndim == 2 and qww.shape[1] == 3 and qww.shape[0] == t_hr.shape[0]:
        ...
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal Interval Qww vs Time, sigma={sigma_opt_m_s2:.3e} m/s^2")
        fig.tight_layout()
        fig.savefig(diag_dir / "ekf_dmc_optimal_qww_vs_time.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    if P9.ndim == 3 and P9.shape[1] >= 9 and P9.shape[0] == t_hr.shape[0]:
        ...
        for i in range(3):
            ...
            axs[i].grid(True, which="both")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF DMC Optimal Pww vs Time, sigma={sigma_opt_m_s2:.3e} m/s^2")
        fig.tight_layout()
        fig.savefig(diag_dir / "ekf_dmc_optimal_pww_vs_time.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
        ...
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.suptitle(f"EKF DMC Optimal sqrt(Pww) vs Time, sigma={sigma_opt_m_s2:.3e} m/s^2")
        fig.tight_layout()
        fig.savefig(diag_dir / "ekf_dmc_optimal_sqrt_pww_vs_time.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
...
def main():
    ...
    J2 = 0.0010826269
    J3 = -2.5324e-6
    all_meas, Xtrue_meas, stations, R, P0, x0_hat, x0_true = load_problem2_inputs(mu_km3_s2=mu)
    ...
    summary_df.to_csv(plot_dir / "ekf_dmc_sweep_summary.csv", index=False)

    if valid_df.empty:
        raise RuntimeError("All EKF DMC sigma runs failed; no valid point available for optimal-case plots.")
    ...
    make_optimal_plots(out_opt, opt_dir, R, sigma_opt_m_s2, tau_s)
