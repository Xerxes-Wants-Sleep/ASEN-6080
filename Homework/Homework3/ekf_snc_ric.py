...
def make_sweep_plots(summary_df: pd.DataFrame, outdir: Path, opt_sigma_m_s2: float) -> None:
    ...
    fig.suptitle("EKF SNC (RIC Q) Sweep: Linear Postfit RMS vs Process Noise Acceleration")
    fig.tight_layout()
    ...
    axs[1].grid(True, which="both")
    axs[1].legend(loc="best")
    fig.suptitle("EKF SNC (RIC Q) Sweep: 3D State RMS vs Process Noise Acceleration")
    fig.tight_layout()
...
def make_optimal_plots(out: dict, outdir: Path, R: np.ndarray, sigma_opt_m_s2: float) -> None:
    ...
    fig.suptitle(f"EKF Optimal SNC (RIC Q) State Errors (+/-3sigma), Process Noise Acceleration={sigma_opt_m_s2:.3e}")
    ...
    if np.any(mask):
        ...
        axs[-2].set_xlabel("Time [hours]")
        axs[-1].set_xlabel("Time [hours]")
        fig.suptitle(f"EKF Optimal SNC (RIC Q) State Errors (+/-3sigma), t >= 5 h, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
        handles, leglabels = axs[0].get_legend_handles_labels()
        fig.legend(handles, leglabels, loc="upper right")
        fig.tight_layout()
    ...
    fig.suptitle(f"EKF Optimal SNC (RIC Q) Linear Postfit Residuals, Process Noise Acceleration={sigma_opt_m_s2:.3e}")
    fig.tight_layout()
