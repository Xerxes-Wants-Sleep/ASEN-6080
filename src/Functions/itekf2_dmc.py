...
class IEKF2_DMC:
    """
    IEKF/EKF with optional iteration and RTS smoothing, using DMC process noise.

    Expected DMC state layout is [r(3), v(3), w(3), ...] where the process drive
    enters the w-block. This class keeps the IEKF2 interface, but replaces the
    SNC Q build with Van Loan discretization from DMC parameters.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.dmc_state_start = int(dmc_state_start)
        self.dmc_dim = int(dmc_dim)
        if self.dmc_dim <= 0:
            raise ValueError(f"dmc_dim must be > 0; got {self.dmc_dim}")
        if self.dmc_state_start < 0:
            raise ValueError(f"dmc_state_start must be >= 0; got {self.dmc_state_start}")
        if self.dmc_state_start + self.dmc_dim > self.n:
            raise ValueError(
                f"DMC block [{self.dmc_state_start}:{self.dmc_state_start + self.dmc_dim}] "
                f"does not fit state dimension n={self.n}."
            )

        self.tau_s = float(tau_s)
        if self.tau_s <= 0.0:
            raise ValueError(f"tau_s must be > 0; got {self.tau_s}")
        self.beta_1_s = 1.0 / self.tau_s

        self.sigma_accel_m_s2 = float(sigma_accel_m_s2)
        if self.sigma_accel_m_s2 < 0.0:
            raise ValueError(f"sigma_accel_m_s2 must be >= 0; got {self.sigma_accel_m_s2}")
        sigma_accel_km_s2 = self.sigma_accel_m_s2 * 1.0e-3
        self.q_psd_km2_s5 = 2.0 * self.beta_1_s * (sigma_accel_km_s2**2)

        self.B = self.beta_1_s * np.eye(self.dmc_dim, dtype=float)
        self.L = np.zeros((self.n, self.dmc_dim), dtype=float)
        i0 = self.dmc_state_start
        i1 = i0 + self.dmc_dim
        self.L[i0:i1, :] = np.eye(self.dmc_dim, dtype=float)
        self.Qc = self.q_psd_km2_s5 * np.eye(self.dmc_dim, dtype=float)
    ...
    def build_process_noise_dmc(self, delta_t: float, X_lin: np.ndarray, t: float) -> np.ndarray:
        dt = max(float(delta_t), 0.0)
        if dt == 0.0:
            return np.zeros((self.n, self.n), dtype=float)

        A = np.asarray(self.dyn_jac(float(t), np.asarray(X_lin, dtype=float).reshape(-1)), dtype=float)
        if A.shape != (self.n, self.n):
            raise ValueError(f"dyn_jac must return ({self.n},{self.n}); got {A.shape}")

        _, Qk = _van_loan_discretization(A=A, L=self.L, Qc=self.Qc, dt=dt)
        return Qk
    ...
    def run(
        ...
        *,
        ...
        Qk_hist = np.full((N, self.n, self.n), np.nan, dtype=float)
        ...
        for j in range(N):
            ...
            if no_snc_on_first_after_gap and is_gap_step:
                ...
                Qk = self.build_process_noise_dmc(dt, X_lin=Xbar, t=t)
            Pbar = Phi @ P @ Phi.T + Qk
            ...
            if show_progress:
                ...
                if (k == N) or (k % prog_step == 0):
                    ...
                    tag = "IEKF2_DMC" if iterated else "EKF2_DMC"
                    print(
        ...
        ww_diag = np.full((N, self.dmc_dim), np.nan, dtype=float)
        i0 = self.dmc_state_start
        i1 = i0 + self.dmc_dim
        for j in range(N):
            ww_diag[j, :] = np.diag(Qk_hist[j, i0:i1, i0:i1])
        ...
        return {
            ...
            "Qk_hist": Qk_hist,
            "Qk_interval_ww_diag": ww_diag,
            "nis_hist": nis_hist,
            "iter_counts": iter_counts,
            "rms_final": rms_final,
            "rms_by_iter": None,
            "R": self.R.copy(),
            "dmc_tau_s": self.tau_s,
            "dmc_beta_1_s": self.beta_1_s,
            "dmc_sigma_m_s2": self.sigma_accel_m_s2,
            "dmc_q_psd_km2_s5": self.q_psd_km2_s5,
...
def it_ekf2_DMC(
    *,
    ...
    """
    Convenience one-shot runner for IEKF2_DMC.
    """
    filt = IEKF2_DMC(
        x0=x0,
        P0=P0,
        ...
        dyn_jac=dyn_jac,
        tau_s=tau_s,
        sigma_accel_m_s2=sigma_accel_m_s2,
        dmc_state_start=dmc_state_start,
        dmc_dim=dmc_dim,
    ...
    return filt.run(
        ...
        no_snc_on_first_after_gap=no_snc_on_first_after_gap,
